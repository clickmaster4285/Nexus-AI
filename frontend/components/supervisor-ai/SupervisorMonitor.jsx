                  ...
                  {["All", "On Call", "Available", "Wrap Up", "Alerts"].map((f) => (
                     <Button
                        ...
                        className={cn(
                           ...
                           filter === f ? "shadow-lg scale-105" : "border-primary/10"
                        )}
                     >
                        {f} {f === "Alerts" && <Badge className="ml-2 h-4 min-w-4 px-1 bg-red-500 text-white border-none text-[8px]">{supervisorAgents.filter(a => a.alerts.length > 0).length}</Badge>}
                     </Button>
                  ))}
               </div>
               <div className="flex gap-2">
                  <Button variant="outline" size="sm" className="h-8 gap-2 text-[10px] font-black border-primary/10">
                     Layout View
                  </Button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
               {filteredAgents.map((agent) => (
                  <Card key={agent.id} className={cn(
                     ...
                     agent.alerts.length > 0 ? "border-red-500/30 ring-1 ring-red-500/10 shadow-red-500/5 bg-red-500/2" : "bg-card"
                  )}>
                     ...
                     <CardContent className="p-5">
                        <div className="flex items-start gap-4 mb-4">
                           <Avatar className="h-12 w-12 border-2 border-background shadow-lg ring-2 ring-primary/5">
                              <AvatarFallback className="text-md font-black bg-secondary text-primary">{agent.avatar}</AvatarFallback>
                           </Avatar>
                           <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                 <Badge className={cn(
                                    "text-[8px] px-1.5 py-0 font-black uppercase rounded-full border-none",
                                    agent.status === "On Call" ? "bg-blue-500" :
                                       agent.status === "Available" ? "bg-green-500" : "bg-amber-500"
                                 )}>
                                    {agent.status}
                                 </Badge>
                                 <span className="text-[10px] font-mono text-muted-foreground">{agent.duration}</span>
                              </div>
                           </div>
                        </div>
                        ...
                        {agent.status === "On Call" ? (
                           <div className="space-y-4">
                              <div className="p-3 rounded-xl bg-muted/40 border border-primary/5 space-y-2 group-hover:bg-muted/60 transition-colors">
                                 ...
                                 <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex items-center gap-1">
                                       <div className={cn(
                                          ...
                                          agent.sentiment === "Positive" ? "bg-green-500" : "bg-red-500"
                                       )} />
                                       <span className={cn(
                              ...
                              <div className="space-y-1.5 font-bold uppercase text-[9px] text-muted-foreground letter tracking-tighter">
                                 ...
                                 <Progress value={agent.workload} className={cn(
                                    "h-1.5",
                                    agent.workload > 80 ? "bg-red-500" : "bg-primary"
                                 )} />
                              </div>

                              <div className="flex gap-2 pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                 <Button variant="outline" size="sm" className="flex-1 h-8 gap-1 text-[9px] font-black uppercase bg-background shadow-sm hover:border-primary">
                                    <Phone className="h-3 w-3" /> Listen
                                 </Button>
                                 <Button size="sm" className="flex-1 h-8 gap-1 text-[9px] font-black uppercase shadow-md bg-primary hover:bg-primary/90">
